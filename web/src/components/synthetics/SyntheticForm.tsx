...
import type { SyntheticAssertionType, SyntheticCheck, SyntheticCheckInput } from "@/api/synthetics";
...
const ASSERTIONS: SyntheticAssertionType[] = ["none", "contains", "not_contains", "json_path"];
...
function toInput(d: Draft): SyntheticCheckInput {
  ...
  return {
    ...
    assertion_path: d.assertion_type === "json_path" ? d.assertion_path.trim() : "",
...
export function SyntheticForm({ check, onSaved, onCancel, readOnly = false }: SyntheticFormProps) {
  ...
  const errors: SyntheticFormErrors = useMemo(() => {
    ...
    if (parseExpectedStatus(draft.expected_status) === null) e.expected_status = "expectedStatus";
    return e;
  }, [draft]);
  const id = (name: string) => `${uid}-${name}`;
  const err = (field: keyof SyntheticCheckInput) => (submitted ? errors[field] : undefined);
  ...
  const save = useMutation({
    mutationFn: (input: SyntheticCheckInput) => (check ? updateSyntheticCheck(check.id, input) : createSyntheticCheck(input)),
    onSuccess: (saved) => {
  ...
  return (
    ...
    >
      <fieldset disabled={readOnly} className="flex min-w-0 flex-col gap-4">
        <Section title={check ? t("synthetics.form.editTitle") : t("synthetics.form.createTitle")}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field id={id("name")} label={t("synthetics.form.name")} error={message("name")}>
              <Input id={id("name")} value={draft.name} onChange={(e) => update({ name: e.target.value })} {...describedBy(id("name"), message("name"))} />
            </Field>
            <Field id={id("url")} label={t("synthetics.form.url")} error={message("url")}>
              <Input
                id={id("url")}
                value={draft.url}
            ...
            <Field id={id("method")} label={t("synthetics.form.method")}>
              <NativeSelect id={id("method")} value={draft.method} onChange={(e) => update({ method: e.target.value })}>
                {METHODS.map((m) => (
            ...
            <Field id={id("interval")} label={t("synthetics.form.interval")} hint={t("synthetics.form.intervalHint")} error={message("interval_seconds")}>
              <Input
                id={id("interval")}
                inputMode="numeric"
                value={draft.interval_seconds}
                onChange={(e) => update({ interval_seconds: e.target.value })}
                {...describedBy(id("interval"), message("interval_seconds"), t("synthetics.form.intervalHint"))}
              />
            </Field>
            <Field id={id("timeout")} label={t("synthetics.form.timeout")} hint={t("synthetics.form.timeoutHint")} error={message("timeout_ms")}>
              <Input
                id={id("timeout")}
                inputMode="numeric"
                value={draft.timeout_ms}
                onChange={(e) => update({ timeout_ms: e.target.value })}
                {...describedBy(id("timeout"), message("timeout_ms"), t("synthetics.form.timeoutHint"))}
              />
            </Field>
            <Field
              id={id("status")}
            ...
            >
              <Input
                ...
                {...describedBy(id("status"), message("expected_status"), t("synthetics.form.expectedStatusHint"))}
              />
            </Field>
            <Field id={id("headers")} label={t("synthetics.form.headers")} hint={t("synthetics.form.headersHint")} error={message("headers")} className="sm:col-span-2">
              <textarea
                id={id("headers")}
                rows={2}
                value={draft.headers}
                ...
                {...describedBy(id("headers"), message("headers"), t("synthetics.form.headersHint"))}
              />
            </Field>
            {sendsBody && (
              <Field id={id("body")} label={t("synthetics.form.body")} className="sm:col-span-2">
                <textarea
                  id={id("body")}
                  rows={2}
                  value={draft.body}
            ...
            <Field id={id("assertion")} label={t("synthetics.form.assertionType")}>
              <NativeSelect
                id={id("assertion")}
                value={draft.assertion_type}
                onChange={(e) => update({ assertion_type: e.target.value as SyntheticAssertionType })}
              >
                {ASSERTIONS.map((a) => (
            ...
            {draft.assertion_type === "json_path" ? (
              ...
              >
                <Input
                  ...
                  onChange={(e) => update({ assertion_path: e.target.value })}
                  {...describedBy(id("path"), message("assertion_path"), t("synthetics.form.assertionPathHint"))}
                />
              </Field>
            ) : (
            ...
            {draft.assertion_type !== "none" && (
              ...
              >
                <Input
                  ...
                  onChange={(e) => update({ assertion_value: e.target.value })}
                  {...describedBy(id("value"), message("assertion_value"), t("synthetics.form.assertionValueHint"))}
                />
              </Field>
            )}
            <Field id={id("enabled")} label={t("synthetics.form.enabled")} hint={t("synthetics.form.enabledHint")}>
              <input
                id={id("enabled")}
                type="checkbox"
                ...
                {...describedBy(id("enabled"), undefined, t("synthetics.form.enabledHint"))}
              />
            </Field>
          </div>
        </Section>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {readOnly && <p className="text-sm text-muted-foreground">{t("synthetics.form.readOnly")}</p>}
