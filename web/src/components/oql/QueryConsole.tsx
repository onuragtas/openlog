...
import { ChartBar, Clock, LayoutDashboard, Play, Table2, Trash2 } from "lucide-react";
...
const EXAMPLE_QUERIES = [
  "SELECT count(*) FROM Log FACET severity TIMESERIES",
  "SELECT count(*) FROM Log WHERE severity = 'ERROR' FACET service.name",
  "SELECT percentile(duration.ms, 50, 95), count(*) FROM Transaction FACET transaction.name LIMIT 5",
  "SELECT average(value) FROM Metric WHERE metricName = 'system.cpu.utilization' FACET host.name TIMESERIES AUTO",
  "SELECT histogram(duration.ms, 1000, 20) FROM Transaction",
  "SELECT count(*) FROM Transaction TIMESERIES COMPARE WITH 1 day ago",
];
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="flex min-w-0 flex-col gap-4">
          <section aria-label={t("oql.console.editorSection")} className="flex min-w-0 flex-col gap-2 rounded-xl border bg-card p-3">
            <OqlEditor value={text} onChange={setText} onRun={() => execute()} label={t("oql.console.editorLabel")} placeholder={EXAMPLE_QUERIES[0]} minRows={4} />
              ...
              <div className="flex flex-col gap-3">
                ...
                <ul className="flex flex-col gap-1">
                  {EXAMPLE_QUERIES.map((q) => (
                    <li key={q}>
                      <button type="button" onClick={() => execute(q)} className="w-full rounded-md px-2 py-1.5 text-left font-mono text-xs break-words hover:bg-accent pointer-coarse:py-2.5">
                        {q}
                      </button>
                    </li>
                  ))}
