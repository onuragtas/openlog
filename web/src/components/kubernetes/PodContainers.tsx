...
export function PodContainers({ pod }: { pod: Pick<KubernetesPodDetail, "containers"> }) {
  ...
  return (
    <div className="min-w-0 rounded-xl border bg-card">
      <Table data-testid="k8s-pod-containers">
        <TableHeader>
          <TableRow>
          ...
          {pod.containers.map((c) => {
            ...
            return (
              <TableRow key={c.name}>
                ...
                <TableCell className="text-right font-mono tabular-nums">{c.restarts}</TableCell>
                <TableCell className="text-right font-mono tabular-nums">{formatCores(c.cpu_usage, locale)}</TableCell>
                <TableCell className="text-right font-mono tabular-nums">{c.memory_working_set == null ? "–" : formatBytes(c.memory_working_set)}</TableCell>
                ...
                <TableCell className="max-w-80 truncate font-mono text-xs" title={c.image}>
                  {c.image || "–"}
                </TableCell>
              </TableRow>
            );
