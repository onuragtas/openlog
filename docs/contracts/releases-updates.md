...
- `component` values: `infra-agent`, `backend`, `php-agent` (php-agent.md §7.1: `openlog-php-agent_<v>_linux_<arch>.tar.gz`
  ...
  and `dotnet-agent` (`OpenLog.Agent.<v>.nupkg`), all three with `os`/`arch` `any`, and `compose`
  (`openlog-compose-<v>.tar.gz`, `os`/`arch` `any`, `format` `tar.gz`: one top directory `openlog-compose-<v>/` with the
  git-tracked files of `deploy/compose` — `docker-compose.yml` stamped `x-openlog-compose-version: "<v>"`,
  `.env.example`, `clickhouse/`, `README.md`; regular files and directories only, never `.env`, `backups/` or
  `releases/`; installed by install-server.sh and kept in sync by `openlog-updater`, D-111). `format`: `tar.gz`, `deb`,
  `rpm`, `apk`, `jar`, `tgz`, `whl`, `nupkg`. Consumers ignore components and formats they do not know, so new ones do
  not change `schema`.
...
- **Install methods:** `tarball` (macOS `install.sh` and the `.pkg`), `zip` (`install.ps1`), `msi` (registry `HKLM\SOFTWARE\openlog\infra-agent` `InstallMethod=msi`). **Embedded manifests (D-113):** the MSI (amd64, arm64) and the macOS pkg cannot contain the published `manifest.json` (it lists their own digest), so like deb/rpm they install a second manifest of the same version into `versions/<v>/`, signed with the release key(s) and listing only the zip (Windows) or tar.gz (macOS) archives; `schema`, `product`, `version`, `channel`, `released_at`, `notes_url`, `compatibility` and `migrations` are identical to the published manifest (checked by the release job), `images` are omitted. `-reconcile -reconcile-context package` (MSI custom action, pkg postinstall) verifies it with the binary's keys and records a note when it is missing or invalid (MSIs built before D-113: a backend-ordered rollback from such a version fails rule 5 until a self-update installed a version with its manifest). Automatic rollback of an unconfirmed candidate never needs it. Context `package` on macOS also points `current` at the package's version unless a newer self-updated version is current.
