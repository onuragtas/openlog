...
| `OPENLOG_RUM_GEO_HEADER` | `` | Header a trusted proxy or CDN writes the visitor's ISO 3166-1 alpha-2 country into (e.g. `CF-IPCountry` behind Cloudflare), recorded as `geo.country.iso_code` on RUM spans ([rum.md](rum.md) §3.7). Empty = no country recorded. openlog resolves no addresses itself and **never stores a visitor address**; without a proxy that writes the header the field stays empty. Values that are not two letters, and the CDN placeholders `XX` and `T1`, are discarded |
| `OPENLOG_STORAGE_COLD_AFTER_DAYS_PROFILES` | `3` | Days before profiling samples move to the cold volume. Profiles are the widest rows openlog stores, so they move earlier than the other classes |
