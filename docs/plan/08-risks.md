...
| **Compose proje adı çakışması** (`deploy/compose/docker-compose.yml` içinde `name: openlog`) | `-p` vermeden bu dosyayla çalışan bir test/ajan komutu paylaşılan yerel openlog'u (volume'larıyla) silebilir — 2026-09-13'te demo sistemi böyle kayboldu | Paylaşılan yerel openlog tek proje `openlog`; tüm test/demo araçları kendi `-p`'sini zorunlu verir (e2e, mixed-version, updater, stack-demo zaten veriyor); agent/demo uygulamaları ayrı projede (`test/localagents`) ve yalnızca `openlog_default` ağına bağlanır; Makefile `-p openlog`'u açıkça verir |
