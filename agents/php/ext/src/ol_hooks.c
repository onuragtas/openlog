...
static const ol_hook *ol_hook_resolve(zend_function *fn, uintptr_t *code)
{
	...
	*code = 1;
	if (name == NULL || (fn->common.fn_flags & ZEND_ACC_CLOSURE)) {
		return NULL;
	}
	if (scope) {
...
{
	return OLG(active) && !OLG(in_call) && (OLG(recording) || (h->flags & (OL_HF_PROPAGATE | OL_HF_ANY)));
}
...
{
	const ol_hook *h = OLG(active) ? ol_hook_lookup(ex->func) : NULL;
	if (h && h->begin && ol_hook_runs(h)) {
		h->begin(ex, h);
	}
...
#endif
#if PHP_VERSION_ID < 80200
	if (zend_execute_internal == ol_execute_internal) {
