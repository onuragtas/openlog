...
| `php-fpm` | php-fpm | every pool of the PHP-FPM configuration (`listen` and `pm.status_path` of `/etc/php/*/fpm/pool.d/*.conf`, `/etc/php-fpm.d/*.conf`, Remi, cPanel, Plesk), each status page read over FastCGI on its own socket, one resource per pool; pools that answer are kept when others fail (`partial`), and when none answers the reason — no `pm.status_path`, or a 0660 socket the agent may not connect to — is `needs_configuration` with the fix in the hint | none | `phpfpm.*` |
