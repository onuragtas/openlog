...
| `php-fpm` | php-fpm | the pool status page over FastCGI on the pool's own socket or TCP port (`pm.status_path`: `/status`, `/fpm-status`, … are tried); a pool without the setting, or whose 0660 socket the agent may not connect to, is `needs_configuration` with the fix in the hint | none | `phpfpm.*` |
