...
// Verified is a profiler release that passed Verify.
...
// Verify applies the agent update rules to a profiler release (releases-updates.md §3): signature by a trusted
// key, manifest version = target, an ebpf-profiler tar.gz artifact for the platform, and for a downgrade
// target >= rollback_floor of the installed version's manifest. installed returns the manifest kept in the
...
func Verify(manifest, signature []byte, trusted []ed25519.PublicKey, target, osName, arch string, installed func() (*lib.Manifest, error)) (*Verified, error) {
	...
	art, ok := m.Artifact(lib.ComponentEBPFProfiler, osName, arch, lib.FormatTarGz)
	if !ok {
		return nil, fmt.Errorf("release %s has no %s %s/%s %s artifact", m.Version, lib.ComponentEBPFProfiler, osName, arch, lib.FormatTarGz)
	}
	if installed != nil {
		cur, err := installed()
		if err != nil {
			return nil, fmt.Errorf("manifest of the installed profiler: %w", err)
		}
