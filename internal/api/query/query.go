...
var (
	...
	// MetricExemplars holds the trace links of metric data points (schema 0092_metric_exemplars, D-130).
	MetricExemplars    = Table{"metric_exemplars"}
	Logs               = Table{"logs"}
	Spans              = Table{"spans"}
	...
	// Profiles holds continuous profiling samples (schema 0095_profiles): one row per sample, so a flame
	// graph is `SELECT stack, sum(value) GROUP BY stack` and every read must aggregate.
