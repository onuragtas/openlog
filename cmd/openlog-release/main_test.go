...
func TestClassify(t *testing.T) {
	cases := map[string]*lib.Artifact{
		...
		// The whole-host profiler. Without these the packages are built and published and then skipped
		// silently by build-manifest, so install.sh cannot verify them and --with-ebpf-profiler fails.
		"openlog-ebpf-profiler_0.4.0_linux_amd64.deb":    {Component: "ebpf-profiler", OS: "linux", Arch: "amd64", Format: "deb"},
		"openlog-ebpf-profiler_0.4.0_linux_arm64.rpm":    {Component: "ebpf-profiler", OS: "linux", Arch: "arm64", Format: "rpm"},
		"openlog-ebpf-profiler_0.4.0_linux_amd64.tar.gz": {Component: "ebpf-profiler", OS: "linux", Arch: "amd64", Format: "tar.gz"},
		"openlog-ebpf-profiler_0.3.0_linux_amd64.deb":    nil,
